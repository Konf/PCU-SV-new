...
Давайте теперь добавим в модуль входные и выходные сигналы. Входные сигналы обозначаются ключевым словом `input`, выходные - словом `output`. Сами порты модуля (входы `a`, `b`, `c` и выход `y`) перечисляются через запятую в формате, показанном в листинге кода N. Обратите внимание на то, что все входные и выходные порты объявлены как `logic`.
Тип `logic` в SystemVerilog является основным для представления (как одноразрядных, так и многобитных) сигналов и может моделировать как провод (wire), так и регистр (reg). Для портов модуля по умолчанию используется он.

Небольшая справка - В классическом Verilog для портов модулей и внутренних сигналов использовались типы wire (для комбинационных цепей) и reg (для сигналов, которым присваивают значение в процедурных блоках). SystemVerilog вводит универсальный тип logic, который заменяет оба этих типа в большинстве случаев, упрощая описание.`logic` — это не "провод" или "регистр", а тип данных для моделирования 4-значной логики (0, 1, X, Z). 
Внутри always_comb он ведёт себя как wire (создаётся комбинационная цепь).
Внутри always_ff он ведёт себя как reg (создаётся элемент памяти: триггер или защёлка).
При использовании с assign — как wire.
Ключевая мысль: `logic` устраняет путаницу wire/reg из Verilog, но понимание, где возникает комбинационная логика, а где триггер, остаётся за инженером (через выбор always_comb или always_ff)
...
Описанный выше модуль можно представить как некоторый "ящик", в который входит 2 провода с названиями `a` и `b` и из которого выходит один провод с названием `result`. Внутри этого блока результат выполнения операции "И" (в синтаксисе SystemVerilog записывается как `&`) над входами соединяется с выходом.
...
Описание, приведённое выше, определяет `y`, как таблично-заданную функцию, которая равна нулю на наборах (0, 2, 5, 6, 7) - десятичное представление x_bus и единице на всех остальных наборах.
...
После создания шины `x_bus` ей присваивается значение, полученное путем конкатенации (объединения) сигналов `x2`, `x1` и `x0` с помощью оператора `assign`. В языке SystemVerilog для конкатенации используется фигурные скобки `{}`:

```systemverilog
assign x_bus = {x2, x1, x0}; // x_bus[2] = x2, x_bus[1] = x1, x_bus[0] = x0
...
Пример использования конкатенации для создания шин любой ширины из отдельных бит или других шин:

```systemverilog
logic [3:0] bus_a = 4'b1010;
logic [1:0] bus_b = 2'b11;
logic [7:0] bus_combined = {bus_a, bus_b, 2'b01}; 
// Результатом будет bus_combined = 8'b10101101

```

Также с помощью конкатенации можно менять порядок бит или повторять значения:

```systemverilog
logic [3:0] bus_a = 4'b1010;
logic [1:0] bus_b = 2'b11;
logic [3:0] reversed = {bus_a[0], bus_a[1], bus_a[2], bus_a[3]}; // 4'b0101
logic [3:0] repeated = {2{bus_b}}; // 4'b1111
...
При описании аппаратуры с помощью SystemVerilog следует руководствоваться важным правилом: **В блоках always_comb, описывающих комбинационную логику, используют блокирующие присваивания (=), а в блоках always_ff, описывающих последовательностную логику — неблокирующие присваивания (<=)**

Блок `always_ff` исполняется по событию, указанному в списке чувствительности:
`always_ff @(posedge clk)` будет исполняться каждый раз когда произойдет положительный фронт clk.
В SystemVerilog для описания комбинационной логики используется `always_comb`, который не требует явного списка и автоматически является чувствительным ко всем используемым в нём сигналам, что предотвращает ошибки проектирования.
`always_comb` будет исполняться всякий раз, когда изменяется значение любой из переменных, используемых в правой части присваиваний внутри этого блока (или в условиях, если они есть).

Синтезатор видит это всё примерно так, например:
```systemverilog
always_comb begin
	b = a; // b немедленно получает значение a (разумеется, после изменения a)
	c = b; // c получает уже ОБНОВЛЕННОЕ значение b (то есть значение a)
end
```
Последующие операторы в том же блоке не выполняются, пока не завершится текущее блокирующее присваивание (отсюда и название). В комбинационной логике `always_comb` ведет себя интуитивно понятно: сигналы обновляются последовательно, как в обычном программировании.

Если же я напишу так:
```systemverilog
...
Перед выполнением, рекомендуется ознакомиться с тем, как работать в Vivado (Создание нового проекта под Nexys A7, менеджер проекта, как запустить симуляцию, прошить ПЛИС и т.д) - [Основы работы в Vivado](https://github.com/MPSU/APS/tree/master/Vivado%20Basics)

Общая структурная схема проекта выполненной лабораторной работы показана на изображении ниже.

![Общая структурная схема проекта выполненной лабораторной работы](./pic/lab1_sch.svg)
...
```systemverilog
logic [7:0] AN; // в 7 артиксе динамическая индикация
