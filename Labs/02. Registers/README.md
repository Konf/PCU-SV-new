...
Механическая кнопка, в связи со своей конструкцией (например, упругусть элементов), при нажатии не просто переходит в состояние логической "1", но перед этим колеблится между двумя уровнями. Данный процесс представлен на схеме ниже.

![Пример дребезга кнопки](./pic/bounce_example.png)

Поскольку период подобных всплесков держится в основном 1-10 мс, высокочастотная схема засчитает не одно переключение, а несколько. В таком случае, нужно проверить, что выставленный сигнал держится не несколько мс, а какое-то время, которое мы признаем как действительное для детектирования.

> [!WARNING]
> Данный пункт со звёздочкой, поскольку рассмотренная снизу схема может оказаться перегруженной для тех, кто только начинает своё знакомство с цифровой логикой. Потому она необязательна к изучению и приводится лишь для ознакомления интересующимся. В лабораторной работе можно ограничиться схемой с двумя регистрами и детектором фронта, что была представлена сверху. Но стоит помнить, что дребезга данной схемой не избежать, и при нажатии на кнопку нет-нет да будут срабатывать ложные нажатия.

На входе уже известный синхронизатор из двух регистров. Далее - сама схема. Если происходит переключение, то счетчик сбрасывается и начинает свой счёт. Как только происходит переполнение - значит необходимое время прошло, и можно записать значение. В конце схемы стоит уже известный генератор синхросигнала.

![Пример дребезга кнопки](./pic/full_debounce.svg)

---

Обработав поступающий сигнал события необходимой схемой, теперь можно подать его с порта `OUT`  к входу `EN` счётчика.

Замечание: рассмотренные схемы работают только тогда, когда входной сигнал изменяется с частотой меньшей, чем частота синхронизации. 

## Настройка Vivado

[Основы работы в Vivado](https://github.com/MPSU/APS/tree/master/Vivado%20Basics)

Для связывания периферии платы с нашим модулем необходимо задать в специальном файле ограничений на дизайн (.xdc - xilinx design constraints) номера пинов (можно посмотреть в документации на плату), которые будут активны во время работы платы, а также задать им имена, чтобы использовать в дальнейшем их в коде. После нужно подключить этот файл в vivado как, соответственно, файл ограничений.

Хорошей практикой является использование модуля верхнего уровня wrapper. Его порты задаются в соответствие именам пинов платы, а в теле подключается необходимый модуль. Иной логики во wrapper нет. Данная практика позволяет, если мы вдруг захотим использовать другую плату, с легкостью выполнить переподключение портов, не меняя сотни строк кода в основном модуле. К тому же, дав адекватные имена для портов нашего блока, читаемость кода повышается. 
...
,   output logic [7:0] AN
,   output logic       CA, CB, CC, CD, CE, CF, CG, CP
);
    ...
    ,   .buf_o      (LEDR     )
    ,   .hex_block_o(AN       )
    ,   .hex_dash_o ({CA, CB, CC, CD, CE, CF, CG, CP})
    );
...
1. Ввод информации происходит с переключателей `SW[9:0]` и кнопок `BTNL`, `BTNR`. Внешний источник сигнала синхронизации: `CLK100MHZ`;
...
logic sw_event;
always_comb begin
  if ((buf_i[0] + buf_i[1] + buf_i[2] + buf_i[3]
     + buf_i[4] + buf_i[5] + buf_i[6] + buf_i[7]
     + buf_i[8] + buf_i[9]) > 4'd3) sw_event = 1'b1;
  else sw_event = 1'b0;
end
...
always_ff @(posedge clk_i or posesge arst_i) begin
